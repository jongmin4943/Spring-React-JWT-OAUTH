...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		//토큰 검증 ( 이게 인증이기때문에 AuthenticationManager도 필요 없음)
		// 내가 SecurityContext에 직접 접근해서 세션 말들때 자동으로 UserDetailsService에 있는
		// loadByUsername이 호출됨
		...
		if(username != null) {
			...
			// 인증은 토큰 검증시 끝. 인증을 위해서가 아닌 시큐리티가 수행해주는 권한 처리르 위해
